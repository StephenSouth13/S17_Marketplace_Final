...
const Shop = ({ categories, brands }: Props) => {
  ...
  // Dùng useCallback để tối ưu hóa và đảm bảo tính ổn định của hàm
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      console.error("LỖI TẢI SẢN PHẨM (Client):", error);
      if (error instanceof Error) {
        console.error("Chi tiết lỗi:", error.message, error.stack);
      } else if (error && typeof error === 'object') {
        // Dùng JSON.stringify để in toàn bộ nội dung nếu không phải Error object tiêu chuẩn
  ...
  useEffect(() => {
    fetchProducts();
  }, [fetchProducts]); // Chỉ cần fetchProducts trong dependency array
